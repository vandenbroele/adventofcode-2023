...
int count_reflections(char **pattern, int rows, int cols, int skip){
	int i, j, k;
	bool reflects;
...
int count_smudge_reflections(char **pattern, int rows, int cols){
	int i, j, original_reflections, reflections;

	original_reflections = count_reflections(pattern, rows, cols, -1);
	for(i = 0; i < rows; i++){
		for(j = 0; j < cols; j++){
			pattern[i][j] = invert(pattern[i][j]);

			reflections = count_reflections(pattern, rows, cols, original_reflections);

			pattern[i][j] = invert(pattern[i][j]);	

			if(0 < reflections && original_reflections != reflections){
				return reflections;
			}
		}
	}
...
void solve(){
	int i, rows, cols, total_reflections;
	char **pattern;
	...
	while(0 < rows){
		...
		total_reflections += count_smudge_reflections(pattern, rows, cols);
		// find horizontal reflection
		pattern = rotate(pattern, &rows, &cols);
		total_reflections += 100 * count_smudge_reflections(pattern, rows, cols);
