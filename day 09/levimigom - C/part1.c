...
int predict_next_value(int *dataset, int dataset_entry_count){
	...
	int next_sequence_value = predict_next_value(sequence, dataset_entry_count - 1);
	free(sequence);

	return dataset[dataset_entry_count - 1] + next_sequence_value;
}

int main(){
	int i, next_value_sum, *dataset, dataset_entry_count;

	next_value_sum = 0;
	dataset = read_dataset(&dataset_entry_count);
	while(1 < dataset_entry_count){
		next_value_sum += predict_next_value(dataset, dataset_entry_count);
